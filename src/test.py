...
def project_polygon(polygon):
    ...
    for point in polygon:
        ...
        x = x_angle / math.radians(FOV) * \
            screen.get_width() + screen.get_height() // 2
        y = y_angle / math.radians(FOV) * \
...
def generate_poly_row(y):
    ...
    for x in range(30):
        ...
        # handle coloring
        if water:
            c = (0, min(255, max(0, 150 - depth * 25)),
                 min(255, max(0, 255 - depth * 25)))
        else:
...
while True:
    ...
    for i, polygon in enumerate(polygons):
        ...
        d = polygon[0][0][1]
        if d < 5:
            pygame.draw.polygon(display, (min(max(0, d * 20) + 150, 255), min(
                max(0, d * 20) + 150, 255), min(max(0, d * 20) + 150, 255)), poly2)
